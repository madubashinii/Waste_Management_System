...
export default function RouteStats({ route }) {
    ...
    const completionRate = Math.round((collected / totalStops) * 100);

    const stats = [
        { label: 'Total Stops', value: totalStops, icon: MapPin, bgColor: 'bg-blue-50', iconColor: 'text-blue-600', borderColor: 'border-blue-200' },
        { label: 'Collected', value: collected, icon: CheckCircle, bgColor: 'bg-green-50', iconColor: 'text-green-600', borderColor: 'border-green-200' },
        { label: 'Pending', value: pending, icon: Clock, bgColor: 'bg-yellow-50', iconColor: 'text-yellow-600', borderColor: 'border-yellow-200' },
        { label: 'Completion Rate', value: `${completionRate}%`, icon: TrendingUp, bgColor: 'bg-emerald-50', iconColor: 'text-emerald-600', borderColor: 'border-emerald-200' }
    ];
        ...
        <>
            <div className="bg-gradient-to-r from-emerald-600 to-teal-600 rounded-xl shadow-lg p-6 mb-8 text-white">
                <div className="grid md:grid-cols-4 gap-6">
                    <div>
                        ...
                        <p className="text-xl font-bold">{route.routeName || 'Route ' + route.route_id}</p>
                    </div>
                    <div>
                        <p className="text-emerald-100 text-sm mb-1">Zone</p>
                        <p className="text-xl font-bold">{route.zone || 'N/A'}</p>
                    </div>
                    <div>
                        <p className="text-emerald-100 text-sm mb-1">Date</p>
                        <p className="text-xl font-bold">{route.date ? new Date(route.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) : 'Today'}</p>
                    </div>
                    <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {stats.map((stat, index) => {
                    ...
                    return (
                        <div key={index} className={`${stat.bgColor} border-2 ${stat.borderColor} rounded-xl shadow-md p-6 transition-transform hover:scale-105`}>
                            <div className="flex items-center justify-between mb-4"><Icon className={`w-8 h-8 ${stat.iconColor}`} /></div>
